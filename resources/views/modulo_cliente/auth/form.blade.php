    ...
    <div class="col-12 col-sm-5 col-md-4 col-lg-5">
      <table class="table table-dark text-light">
        ...
        <tbody>
          ...
          <tr>
            <td colspan="2">Sin registros de abogado</td>
          </tr>
          @else

          @foreach($abogados as $ab)
          <tr>
            <td class="text-center">{{$ab->IDNRO}}</td> 
            <td class="text-center" >{{$ab->NOMBRES}}</td>
            @php
            $checar= $ab->MARCAR == 1 ? "checked"  : "";
            @endphp 
            <td class="text-center" > <input {{$checar}} type="checkbox" name="ABOGADO[]" value="{{$ab->IDNRO}}"> </td>
          </tr>
          @endforeach
...
<script>
  ...
  function ajaxCall(ev) { //Objeto event   DIV tag selector to display   success handler
    ...
    $.ajax({
      ...
      headers: {
        'Content-Type':'application/x-www-form-urlencoded',  'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
      },
      beforeSend: function() {
